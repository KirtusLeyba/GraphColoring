...
//a graph coloring GA
public class balGraphColoringGA extends GeneticAlgorithm
{
	private int numColors;
	private int numNodes;
	private Graph g;

	public balGraphColoringGA(int k, double p, double m, int popSize, int numColors, int numNodes, Graph g)
    {
    ...
    //In this case, the "balanced" fitness function for graph coloring
    public double fitnessFunction(int[] individual)
    {
    	double fitness = 0.0;
    	int edgeCount = 0;
        double[] numPerColor = new int[numColors];
        ...
        double balance = numPerColor[0];
        for(int i = 1; i < numColors; i++)
        {
            balance = balance*numPerColor[i];
        }
        balance = balance/numNodes;
        double maxF = Math.pow(numColors, numColors)/Math.pow(numNodes, numColors);
        fitness = fitness/edgeCount;

    	return fitness*balance/maxF;
    }
